...
void multiStageGraph(int stageNum, int* numPerStage)
{
    ...
    memset(minRoad, 0x3f, sizeof(minRoad)); //初始化为一个充分大的数 0x3f
    ...
    {
        for (q = 0; q < numPerStage[i]; ++q) //遍历第 i 段顶点
        {
            for (p = 0; p < numPerStage[i + 1]; ++p) //遍历第 i+1 段顶点
            {
                scanf("%d", &weight); //读取边(q,p)的权重 w(q,p)
                if (weight != -1) //存在边(q,p)
                {
                    temp = minRoad[i][q] + weight;
                    if (temp < minRoad[i + 1][p]) //发现 s 到 o 的更短路径
                    {
                        minRoad[i + 1][p] = temp;
                    }
                }
